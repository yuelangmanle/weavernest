...
The `0.3.1-alpha` creator builds with the local JDK 17, Gradle 8.10.2, and Android SDK 35 toolchain under `.local`. It includes the paste-to-editor workflow, persistent WebView preview with explicit run controls and runtime logs, core capability registry, `weaver-required` import recognition, bilingual Prompt Pack 2.0, encrypted AI settings, supplied launcher icon, GitHub Releases update download states, Apilot V2 package visibility and confirmation guidance, and signed creator APK output. The preview bootstrap now creates `window.weaver` before project scripts; native capability calls still report explicit `UNSUPPORTED` states until the Android 10+ Runtime 2.0 matrix passes.
