...
The `0.3.0-alpha` creator builds with the local JDK 17, Gradle 8.10.2, and Android SDK 35 toolchain under `.local`. It includes the paste-to-editor workflow, persistent WebView preview with explicit run controls and runtime logs, core capability registry, import analysis, prompt pack, encrypted AI settings, supplied launcher icon, GitHub Releases update download states, Apilot V2 interoperation, and signed creator APK output. The preview bridge currently exposes persistent data and explicit unavailable states for native operations whose device modules have not passed the Android 10+ matrix yet.
...
- The creator home offers AI creation, direct code paste/import, and templates. The paste screen reads the Android clipboard or files, identifies HTML/CSS/JavaScript shape and device-API conversion suggestions, and requires the user to confirm creation before it writes a project. A workspace has dedicated AI, Files, Preview, Capabilities, Build, and Data areas. Files use an offline native monospace editor with file switching and automatic save; locally packaged CodeMirror resources remain a later enhancement rather than a network dependency.
